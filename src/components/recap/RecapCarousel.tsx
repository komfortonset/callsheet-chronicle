    ...
    >
      ...
      <div className="absolute top-4 left-4 right-4 flex gap-2 z-50">
        {slides.map((_, index) => (
          <div
            key={index}
            className="h-1 flex-1 bg-white/30 rounded-full overflow-hidden backdrop-blur"
          >
            <div
              className={cn(
                "h-full bg-white rounded-full transition-all duration-300",
                index < currentSlide && "w-full",
                index === currentSlide && "w-full animate-progress",
      ...
      <div className="absolute inset-0 flex pointer-events-none z-40">
        <button
          ...
          className="w-1/3 h-full pointer-events-auto opacity-0 disabled:pointer-events-none"
